...
#            side
            units = 1
            if upstreakCount > 2 and devUp>devDown and upTick>downTick:
               # Buy
                stopLoss = bid[-1]-spread
                takeProfit = ask[-1]+2.5*spread
#                time = datetime.date.today()
                order = OrderEvent(instrument, units, "market", 'buy', bid[-1], ask[-1], time[-1], stopLoss, takeProfit)
                self.events.put((1,time[-1],order))
#                print('buy')
        
            if downstreakCount > 2 and devUp<devDown and upTick<downTick:
               # Buy
                stopLoss = ask[-1]+spread
                takeProfit = bid[-1]-2.5*spread
#                time = datetime.date.today()
